...
import { getPassengerById, updatePassengerById } from 'apiSdk/passengers';
...
function PassengerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PassengerInterface>(
    () => (id ? `/passengers/${id}` : null),
    () => getPassengerById(id),
  );
  ...
  const handleSubmit = async (values: PassengerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePassengerById(id, values);
      mutate(updated);
      resetForm();
