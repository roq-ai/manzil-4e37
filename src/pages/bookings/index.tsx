...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBookings({
        relations: ['passenger', 'driver', 'vehicle'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('passenger', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/passengers/view/${record.passenger?.id}`}>
              {record.passenger?.id}
            </Link>
          ),
        }
    ...
    hasAccess('driver', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          accessorKey: 'driver',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/drivers/view/${record.driver?.id}`}>
              {record.driver?.current_location}
            </Link>
          ),
        }
      : null,
